"""Fix AC-PBLINI and CH-GAOP: remove + refund paid items.

Usage:
    python fix_pblini_gaop.py              # dry-run
    python fix_pblini_gaop.py --commit     # apply
"""
...
def remove_item(order_info):
    ...
    for edge in calc_order["lineItems"]["edges"]:
        node = edge["node"]
        if (node.get("sku") or "").strip() == sku and node["quantity"] > 0:
            li_node = node
            break
    ...
    mutation orderEditCommit($id: ID!) {
      orderEditCommit(id: $id, notifyCustomer: false, staffNote: "Remove %s (paid, out of stock)") {
        order { id name }
        userErrors { field message }
      }
    }
...
def refund_item(order_info):
    ...
    if order_info["price"] <= 0:
        print(f"    $0 item, no refund needed")
        return True
    try:
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  Fix {', '.join(TARGET_SKUS)} [{mode}]")
    ...
    for sku in TARGET_SKUS:
        sku_paid = [p for p in paid if p["sku"] == sku]
        sku_cur = [c for c in curation if c["sku"] == sku]
        total_refund = sum(p["price"] * p["qty"] for p in sku_paid)
        print(f"\n  {sku}:")
        print(f"    Paid (remove+refund): {len(sku_paid)} orders, ${total_refund:.2f}")
        print(f"    Curation (skip): {len(sku_cur)} orders")
        if sku_paid:
            for p in sku_paid:
                print(f"      {p['order_name']} x{p['qty']} ${p['price']:.2f}")

    if not COMMIT:
        print(f"\nDRY-RUN. {len(paid)} total would be removed+refunded.")
