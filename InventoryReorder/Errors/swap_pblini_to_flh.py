...
Usage:
    python swap_pblini_to_flh.py              # dry-run
    python swap_pblini_to_flh.py --commit     # apply
"""
...
def swap_item(order_info, variant_gid):
    ...
    mutation orderEditCommit($id: ID!) {
      orderEditCommit(id: $id, notifyCustomer: false, staffNote: "Swap AC-PBLINI -> AC-FLH (curation, out of stock)") { order { id name } userErrors { field message } }
    }""", {"id": calc_id})
...
def main():
    ...
    print(f"{'='*60}\n")
    print("Looking up AC-FLH variant...")
    vgid = find_variant()
    if not vgid:
        print("  ERROR: variant not found!")
        return
    print("\nFetching curation AC-PBLINI orders...")
