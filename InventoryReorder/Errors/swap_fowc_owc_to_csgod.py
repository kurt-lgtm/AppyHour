...
Usage:
    python swap_fowc_owc_to_csgod.py              # dry-run
    python swap_fowc_owc_to_csgod.py --commit     # apply
"""
...
vdata = gql('{ productVariants(first: 5, query: "sku:CH-CSGOD") { edges { node { id sku price product { title } } } } }')
variants = [e["node"] for e in vdata["productVariants"]["edges"] if e["node"]["sku"] == "CH-CSGOD"]
variants.sort(key=lambda v: float(v["price"]))
CSGOD_GID = variants[0]["id"]
print(f"  CH-CSGOD: {CSGOD_GID} ${variants[0]['price']}")

# Fetch OWC curation CH-FOWC orders
...
while url:
    ...
    for o in resp.json().get("orders", []):
        ...
        if is_owc and has_fowc_curation:
            targets.append({
...
print(f"  Found {len(targets)} OWC CH-FOWC orders (limit {SWAP_LIMIT})")

mode = "COMMIT" if COMMIT else "DRY-RUN"
if not COMMIT:
    print(f"\nDRY-RUN. {len(targets)} would be swapped.")
...
for t in targets:
    ...
    }""", {"id": calc_id, "variantId": CSGOD_GID, "quantity": 1})
    if data["orderEditAddVariant"]["userErrors"]:
        f += 1; continue
    time.sleep(0.3)
    data = gql("""
    mutation orderEditCommit($id: ID!) {
      orderEditCommit(id: $id, notifyCustomer: false, staffNote: "Swap CH-FOWC -> CH-CSGOD (OWC, out of stock)") { order { id name } userErrors { field message } }
    }""", {"id": calc_id})
