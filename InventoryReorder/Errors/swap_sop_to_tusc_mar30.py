...
"""Swap curated MT-SOP -> MT-TUSC on _SHIP_2026-03-30 orders.

Only swaps items with _rc_bundle property (curation items, not paid add-ons).

Usage:
    python swap_sop_to_tusc_mar30.py              # dry-run
    python swap_sop_to_tusc_mar30.py --commit     # apply
"""
    ...
    while url:
        ...
        for o in resp.json().get("orders", []):
            ...
            if SHIP_TAG not in tags:
                continue
            # Skip if order already has MT-TUSC (paid or chosen — would create duplicate)
            all_skus = {(li.get("sku") or "").strip() for li in o.get("line_items", [])
                        if li.get("fulfillable_quantity", li.get("quantity", 0)) > 0}
            if NEW_SKU in all_skus:
                continue
...
def swap_order(order_info, variant_gid):
    ...
    if data["orderEditSetQuantity"]["userErrors"]:
        print(f"    FAILED setQty: {data['orderEditSetQuantity']['userErrors']}")
        return False
    ...
    if data["orderEditAddVariant"]["userErrors"]:
        print(f"    FAILED addVariant: {data['orderEditAddVariant']['userErrors']}")
        return False
    ...
    mutation orderEditCommit($id: ID!) {
      orderEditCommit(id: $id, notifyCustomer: false, staffNote: "Swap MT-SOP -> MT-TUSC (curation, out of stock)") {
        order { id name }
        userErrors { field message }
      }
    }""", {"id": calc_id})
    if data["orderEditCommit"]["userErrors"]:
        print(f"    FAILED commit: {data['orderEditCommit']['userErrors']}")
        return False
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  Curated {OLD_SKU} -> {NEW_SKU} [{mode}]")
    ...
    print(f"Fetching curated {OLD_SKU} orders tagged {SHIP_TAG}...")
    targets = fetch_targets()
    print(f"  {len(targets)} curation orders found\n")
    for t in targets:
