"""Swap curation MT-BRAS to MT-SOP on _SHIP_2026-03-23 orders.

Usage:
    python swap_bras_to_sop.py              # dry-run
    python swap_bras_to_sop.py --commit     # apply
"""
...
def find_variant():
    query = '{ productVariants(first: 5, query: "sku:MT-SOP") { edges { node { id sku title price product { title } } } } }'
    data = gql(query)
    variants = []
...
def swap_item(order_info, variant_gid):
    ...
    mutation orderEditCommit($id: ID!) {
      orderEditCommit(id: $id, notifyCustomer: false, staffNote: "Swap MT-BRAS -> MT-SOP (curation, out of stock)") { order { id name } userErrors { field message } }
    }""", {"id": calc_id})
...
def main():
    ...
    print(f"{'='*60}\n")
    print("Looking up MT-SOP variant...")
    vgid = find_variant()
    if not vgid:
        print("  ERROR: variant not found!")
        return
    print("\nFetching curation MT-BRAS orders...")
    targets = fetch_targets()
    print(f"  Found {len(targets)} curation orders\n")
    if not COMMIT:
