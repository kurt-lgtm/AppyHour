...
def fix_order(r):
    ...
    # Debug: show all CEX-EC lines in edit session
    calc_cex = []
    for e in edit["calculatedOrder"]["lineItems"]["edges"]:
        node = e["node"]
        if node["sku"] == target_sku:
            calc_cex.append(node)
            print(f"    Found: {node['sku']} qty={node['quantity']} id={node['id']}")

    if not calc_cex:
        print(f"    No {target_sku} found, skipping")
        return False

    time.sleep(0.3)

    # Shopify merges duplicate line items — so we may see qty=2 on a single line
    # Set each to qty=1, or if multiple lines exist remove extras
    edited = False
    for i, node in enumerate(calc_cex):
        if i == 0:
            # Keep first line, but set qty to 1 if it's higher
            if node["quantity"] > 1:
                data = gql("""
                ...
                """, {"id": calc_id, "lineItemId": node["id"], "quantity": 1})
                errors = data["orderEditSetQuantity"]["userErrors"]
                if errors:
                    print(f"    FAILED to set qty: {errors}")
                    return False
                print(f"    Set {target_sku} qty {node['quantity']} -> 1")
                edited = True
                time.sleep(0.3)
        else:
            # Remove extra lines
            ...
            if errors:
                ...
                return False
            print(f"    Removed extra {target_sku}")
            edited = True
            time.sleep(0.3)

    if not edited:
        print(f"    Already qty=1, nothing to change")
        return False
...
for r in results:
    print(f"  {r['order_name']} {r['customer']}: {r['sku']} ({r['rest_count']} lines, total qty {r['total_qty']}) -> keep 1")
