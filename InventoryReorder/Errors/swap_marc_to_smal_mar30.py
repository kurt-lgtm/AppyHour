...
"""Swap curated AC-MARC -> AC-SMAL on _SHIP_2026-03-30 orders.

Excludes orders with AHB-X* or BL-* box SKUs.
Only swaps _rc_bundle (curation) items.

Usage:
    python swap_marc_to_smal_mar30.py              # dry-run
    python swap_marc_to_smal_mar30.py --commit     # apply
"""
...
def fetch_targets():
    ...
    while url:
        ...
        for o in resp.json().get("orders", []):
            ...
            # Exclude AHB-X* and BL-* box orders
            if any(s.startswith("AHB-X") or s.startswith("BL-") for s in active_skus):
                continue
            # Skip if already has AC-SMAL
...
def swap_order(order_info, variant_gid):
    ...
    mutation orderEditCommit($id: ID!) {
      orderEditCommit(id: $id, notifyCustomer: false, staffNote: "Swap AC-MARC -> AC-SMAL (curation, stock sub)") {
        order { id name }
        userErrors { field message }
      }
    }""", {"id": calc_id})
...
def main():
    ...
    print(f"  Ship tag: {SHIP_TAG}  |  Excludes: AHB-X*, BL-*")
    print(f"{'='*60}\n")
