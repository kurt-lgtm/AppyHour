...
12a. **The allocate universe is HAVE ∪ DEMANDED, never HAVE alone** (Kurt 2026-09-11). **Failure that forced the change:** `compute_allocation` asked `_zero_variant_items` only about SKUs present in the HAVE file, so a SKU the cohort demands but the count never listed produced **no row, no shortage, and no push** — its Shopify `available` kept its old value, so customers could go on ordering something we had zero of. `_SHIP_2026-09-14` reported "nothing short" while ten absent SKUs (CH-BLR, MT-IBRES, AC-WASP, MT-PSS, AC-ACRISP, CH-QOTA, CH-TOPR, CH-BBLUE, AC-PMULB, AC-RBOL) were short on the vF; Kurt caught them there, not here. **Absent from the HAVE file means HAVE 0** — the most urgent shortage class, not an exempt one. The union excludes the made-to-order prefixes rule 13 already skips (PK-/MR-/TR-), and the demand-only names come back as `demand_only_skus` so a caller can report "these were never counted" instead of an empty shorts list. Tests: `tests/test_allocate_demand_only.py`.
