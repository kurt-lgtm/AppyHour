...
| C3 | **Current-ship-week late-rate readers** (reship report inputs, cohort scorecard) | `delivery_status` ⨝ `shopify_orders` | **48h** on the CURRENT ship-week window. Existing constant `DELIVERY_COHORT_WINDOW_MAX_H = 48`; the data misled at 19h, 48h is the reader-trust ceiling | current-week joined-window query, ORGANIC rows only (`last_event NOT LIKE 'shopify_events%'`) — 🔴 the whole-table row (now **3d**, was 14d until 2026-08-31) CANNOT catch a dead current-week ingest hiding behind a fresh backfill of old cohorts. 🔴 **And a ZERO on this join is a CLAIM, not a pass** — until 2026-08-31 `n == 0` was graded "pre-ship" unconditionally, which read a total six-day ingest outage as "hasn't shipped yet" on a cohort with 2,414 of 2,469 orders already fulfilled. It now proves the join key by running `fulfillments` through the SAME `REPLACE(order_name,'#','')` expression: shipped>0 with zero delivery rows is a DEAD-INGEST flag, and pre-ship is only reported when zero fulfillments confirm it | `freshness_sweep.py` — ✅ **re-pointed at the cloud via `cloud_reads.connect_reporting()` 2026-08-31** (degrade ⇒ FLAG "UNKNOWN", never "stale"), and the §2.3(3) UTC bug fixed the same day with a per-value `age()` helper (measured +4.00h on a naive `synced_at`, ±0h on an offset-carrying `fulfilled_at`) |
...
🔴 **Nothing was WATCHING it, which is why "15 days dead" was a discovery instead of an alert
(2026-08-31).** `freshness_sweep.py` graded `fulfillments` off the LOCAL sqlite copy, so it proved
`sync_logon.py` alive and stayed green throughout. A monitor pointed at the copy a local script
writes cannot see the copy the cloud serves. Fixed the same day: `CLOUD_TABLE_CHECKS` in the sweep
probes DO directly for `MAX(fulfilled_at)` / `MAX(synced_at)` on a 3d limit — the local rows' own
limit, not a new number. 🔴 It is a **freshness probe only**: reads stay LOCAL, because this
blocker is exactly why they must (moving them loses ship weeks 08-17 and 08-24). An unreachable DO
grades **UNKNOWN and FLAGs** — never green, never "stale" — which is the live state from this
machine today, whose egress is blocked at the DB firewall.
