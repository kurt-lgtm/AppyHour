...
DEFAULT_SETTINGS = "C:/Users/Work/Claude Projects/AppyHour/InventoryReorder/dist/inventory_reorder_settings.json"
DEFAULT_XLSX = "C:/Users/Work/Claude Projects/AppyHour/AHB_WeeklyProductionQuery_03-17-26_vF.xlsx"
DEFAULT_TAG = "RMFG_20260317"
DEFAULT_OUTPUT = "C:/Users/Work/Downloads/ac-swap-list-2026-03-17.csv"
...
def main():
    parser = argparse.ArgumentParser(description="Generate AC-swap CSV from XLSX matrix vs Shopify line items")
    parser.add_argument("--xlsx", default=DEFAULT_XLSX, help="AHB_WeeklyProductionQuery XLSX path")
    parser.add_argument("--settings", default=DEFAULT_SETTINGS, help="inventory_reorder_settings.json path")
    parser.add_argument("--tag", default=DEFAULT_TAG, help="Shopify RMFG tag filter (e.g. RMFG_20260317)")
    ...
    http_headers = {"X-Shopify-Access-Token": token, "Content-Type": "application/json"}
    ...
    for r in range(2, ws.max_row + 1):
        ...
        skus = {}
        for c, name in header_cols.items():
    ...
    while url:
        resp = requests.get(url, headers=http_headers, params=params if not orders else None, timeout=30)
    ...
    print(f"Wrote {len(rows)} swaps to {args.output}")
