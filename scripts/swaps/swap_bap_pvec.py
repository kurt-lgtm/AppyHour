"""Swap CH-BAP -> CH-PVEC on up to 45 orders. BYO suffix orders first.
Protects CH-ALPHA."""
...
# Find orders with CH-BAP (fq>0), separate BYO vs others
byo_orders = []
other_orders = []
for o in all_orders:
    has_bap = False
    ...
    for li in o.get("line_items", []):
        ...
        if sku == OLD_SKU and fq > 0:
            has_bap = True
        if sku.startswith("AHB-") and sku.endswith("-BYO") and fq > 0:
            is_byo = True
    if has_bap:
        if is_byo:
            byo_orders.append(o)
...
print(f"BYO orders with CH-BAP: {len(byo_orders)}")
print(f"Other orders with CH-BAP: {len(other_orders)}")

# BYO first, then others, cap at MAX_SWAPS
targets = (byo_orders + other_orders)[:MAX_SWAPS]
print(f"Orders to swap (capped at {MAX_SWAPS}): {len(targets)}")
...
for o in targets:
    ...
    try:
        ...
        if not bap_li:
            errors.append({"order": name, "error": "No swappable CH-BAP in calculated order"})
            continue
