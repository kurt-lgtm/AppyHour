...
| 5 | **ShipStation API creds** (Account → API Settings → key+secret) OR confirm we go direct FedEx/UPS/OnTrac dev-API OAuth | **INV-2 live carrier TNT** — grounds the data-sparse Express bucket; the council **A2/C1** fix (replaces the optimistic published map + closes survivorship). ShipStation = 1 integration vs 3. | ~5 min |
