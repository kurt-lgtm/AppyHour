...
| **Daily reship tracking report (Dan's Reship Sheet: per-week `RS _SHIP_*` tabs + Summary + Flags)** | `ShippingReports/reship_report_refresh.py [--weeks-back N --dry-run]` (scheduled weekdays 12:18) | 🔒✍️ | Gorgias tag counts (rule 81603 spam — NEVER a reship metric), hand-edited sheet tabs, `fulfillments` as denominator. Constraints SSOT: `ShippingReports/RESHIP_REPORT_RULES.md` (read BEFORE any change). Unit = deduped Shopify reship ORDERS; attribution = original order's cohort (ship-Monday-precedes-complaint guard); denominator = live Shopify `tag:'_SHIP_<Mon>' -status:cancelled`; WoW = same day-offset + tail-CDF projection. State: `_outputs/cache/reship_report_state.json` (freshness-swept, 2d). |
