...
| **EXECUTE SKU swaps** on a ship_tag cohort | `appyhour_swap_order_skus` (dry_run default True). 🔴 Balance invariant enforced 2026-07-10 (#157930 shipped short): every swap path aborts if qty removed ≠ qty added; duplicate-SKU lines all swapped; audits to `swap_audit.jsonl`. NEVER hand-roll an order edit around this. **✅ live-verified 2026-08-28** (`_outputs/logs/swap_audit.jsonl` — wk0831 CH-FENG swaps, result OK). | 🔒✍️ |
| **WEEKLY shorts→swap pass** (whole pairs list, one command: plan → preview → apply → verify) | `scripts/shorts_pass.py SHIP_TAG --pairs pairs.csv [--apply]` — constraints SSOT `scripts/SHORTS_PASS_RULES.md`. Plan via `find_swap_targets` + login-OR-customize exclusion; execution ONLY via `order_edit._swap_order_skus`; verify re-counts `fulfillableQuantity` vs plan (nonzero exit on mismatch — success is NEVER call-count). Dry-run default; JSONL log per run. **✅ live-verified 2026-08-28** (`_outputs/logs/shorts_pass__SHIP_2026-08-31_*.jsonl` — 3 runs incl `--apply`: 7/8 swaps OK, verify phase correctly flagged the one un-editable order as a count mismatch). | 🔒✍️ |
...
|---|---|---|---|
| Weekly RMFG cut order (3-tab xlsx) | `InventoryReorder/build_cut_order_xlsx_v2.py [--have <HAVE.xlsx>]` (or `/cut-order`) — `--have` overrides `settings["corrected_inventory_path"]` for the run; either way the resolved HAVE path + mtime are echoed (stale 6/23-Downloads burn). Base snapshot override: `AH_INV_CSV` env | 🔒✍️ | v1 `build_cut_order_xlsx.py`; `agents/tuesday_cut_order.py` (empty demand); trusting a silent stale `corrected_inventory_path` |
...
| Order checks + swap list for a production run (ALL checks, both guardrail halves, inventory-capped swaps) | `python -m order_checks.run_all --tag <RMFG_/8_24> --ship <_SHIP_> --sheet <vF.xlsx> --have <HAVE csv/xlsx>` (or `/order-checks`) — `--have` = this week's declared HAVE export, REQUIRED (no baked-in fallback: fails loud rather than cap swaps on last week's count) | 🔒 | 🔴 run BEFORE the sheet is built, never after — the login half of login-OR-customize is what wk0831 missed (155 of 543 swapped customers had logged in; sheet already sent). Emits CSVs, writes NOTHING: sheet edits go through `ShipRouting/scripts/vf_edit.py`, Shopify follows the sheet. Never rebuild what was applied from the vf_edit ledger — it is TRUNCATED (659 of 818); diff a backup against the live sheet. Rules: `order_checks/ORDER_CHECKS_RULES.md`. **✅ live-run 2026-08-28** (wk0831 — 818 vf_edit edits applied off its findings; coordination handoff `oc-answer-0831-do-not-reexecute-2026-08-28`) |
| Klaviyo engagement on a swap candidate list (rank, never gate) | `python -m order_checks.klaviyo_opens <swap csv> --tag <RMFG_> [--days 90]` | ✅ | Read-only. NO-OPEN/no-profile first, then newest order (INVENTORY_COORDINATOR rule 9). 🔴 A PRIORITY, not a guard -- login-OR-customize already decided who is swappable; an API error is its own state, never folded into "no open". Metric looked up by NAME; absent = abort, never guess an id. |
| Carrier-invoice ingest → shipping.db | `GelPackCalculator/auto_import.py [--dir]` — 🔴 every `GelPackCalculator/` path in this table is the SIBLING repo `Claude Projects/GelPackCalculator/` since 2026-09-12 (R-35 phase 1), not a subdir of AppyHour; scripts resolve it via `appyhour_lib.paths.gelpack_root()`. Prod copy still `C:\AppyHourProd\AppyHour\GelPackCalculator` until Phase 2. | ✅✍️ | hand SQL inserts; double-import via Kori "Sync All" |
...
| **Scan-derived ORIGIN HUB for a box (PP-native, no invoice/fulfillments join)** | `ShippingReports/build_pp_origin_hub.py` (dry-run DEFAULT; `--apply` writes local `pp_origin_hub`). Callable: `appyhour_lib.pp_origin.derive_origin(payload, canon=canon)`. Constraints SSOT: **`PP_ORIGIN_HUB_RULES.md` — read FIRST.** | 🔒✍️ | 🔴 `checkpoints[0]` (array is NEITHER time-ordered; first 2-3 are PP/store lines → 0 hits on 6,000 payloads) · PP's raw carrier name (says **LaserShip**, must fold to OnTrac via `canon.normalize_carrier`) · a hand-rolled routing-tag regex (`canon.parse_routing_tag` only — a substring test reads a hub out of a `!NO …` block) · `pickup_location` (NULL on 2,694/2,694) · `location.name` (that's the SHIPPER, `RMFG`) · PP `transit_time` (TNT = delivery − pickup SCAN, calendar, ET) · mapping an unknown scan city to a hub (**MISSING**, never a guess). ⚠️ **UNOWNED — no scheduled owner, no freshness assert yet**; needs a `DATA_CANON_RULES.md` declaration (Routing Coordinator's surface) or the Monday sweep FLAGs `[A undeclared]`. Makes **zero** ParcelPanel API calls, READ-ONLY on cloud MySQL. |
...
| Box simulation (DistVol + box size for cohort) | `box_simulation.py [SHIP_TAG] [--refresh] [--distvol <xlsx>]` — DistVol comes from the **DO table `distvol`** (`build_lookup()` → `_distvol_db`, the ONE reader; unreachable = FATAL naming the table). `--distvol <xlsx>` is the ONLY file path (explicit test/ingest override). Every cohort SKU with no `distvol` row prints a named `WARNING: DistVol MISSING` line (sku + orders) — sized on a prefix default, onboarding incomplete | ✅ | inline DistVol regex; the Desktop xlsx / `DISTVOL_XLSX` env as a source (retired 2026-09-11 — Kurt: "not a csv"); reading `PREFIX_DEFAULTS` silently |
| DistVol drift audit | `audit_distvol_drift.py --csv <BoxAssignment csv> --cache <box_sim_cache json> [--out tsv]` — both inputs REQUIRED per run, no baked-in dated paths | 🔒 | re-derive the lookup |
...
| **DB invariants check (DATA_CANON declarations vs LIVE sqlite + cloud MySQL — undeclared tables, grain, key format, empty-means, value domains, account canon)** | `_outputs/scripts/db_invariants_check.py [--store sqlite\|mysql\|both] [--json]` — reads the declarations from ORIGIN (`git show origin/today-experiments-2026-07-03:server/DATA_CANON_RULES.md`), never a checkout; `DATA_CANON_RULES=<file>` overrides for tests; stamps `invariants_runs` only when the checks RAN | ✅ | hand-rolled `SELECT COUNT(*) … GROUP BY key HAVING >1` probes; "declaring" a table by adding a comment. A yaml typo in the doc is a `[lint]` VIOLATION, not a crash (dead 09-01→09-11 once) |
| **Weekly freshness sweep (every local + cloud writer's cadence, unpushed commits, replica age)** | `_outputs/scripts/freshness_sweep.py` (Monday scheduled; `main()` = 16 `stage_*()` fns, each `(flags, ok)`) | ✅ | per-table ad-hoc `MAX(synced_at)` checks; a freshness assert gated on its writer's flag (dead-cadence class) |
| **Cloud → local replica pull (`shopify_orders`, `weather_history` → `*_replica` in shipping.db)** | `_outputs/scripts/pull_cloud_replicas.py` — 3× retry, stamps `replica_pull_log` (declared in DATA_CANON) | ✅✍️ | copying a cloud table by hand; reading a replica without checking `replica_pull_log` age (one failed pull = 9d stale, silently) |
...
| Apply routing tags — main/Monday cohort | `apply.py` (dry-run; `--apply`). **✅ live-verified 2026-08-28** (`_outputs/logs/routing_apply_resolutions.jsonl` run `20260828-171407-0c69af` + paired `routing_decisions.jsonl`) | 🔒✍️ | **Kori Apply button** (ignores engine) |
...
| **ROW/ITEM edits on a vF sheet post-lock** (per-order 1:1 item swap csv / shorts substitution / late add-order from live / drop / set-route / set-item / set-address / bulk retag csv) | `ShipRouting/scripts/vf_edit.py {swap,sub,add-order,drop,set-route,set-item,set-address,retag-csv} --sheet <vF.xlsx>` — in-place atomic write, jsonl ledger `_outputs/logs/vf_edit_<shipdate>.jsonl`, auto-runs presend_check. **✅ live-verified 2026-08-28** (`_outputs/logs/vf_edit_2026-08-31.jsonl` — sub edits on the 08-31 vF; ⚠️ that ledger is TRUNCATED 659/818 per the order-checks row — never rebuild applied state from it) | 🔒✍️ | 🔴 chat-driven scratch openpyxl one-shots (the wk0817 ~5h seam: check7_swap/add11/tray_swap/closer_hub/fix_totals — this replaces them); Excel hand-edits; rewriting bare `!ANY` rows in bulk; inventing an MFG header. Constraints SSOT: `ShipRouting/VF_EDIT_RULES.md`. Tag-only edits stay with `vf_tags.py` |
| **RUN EVERY STANDING CHECK on a vF sheet** (MFG headers, zip5 text, ice/gel, guides, totals, routing legality+coverage, live Shopify cross-check for cancelled/_HOLD/drift-in/SKU diff, name chars, dupe list — then presend_check) | `ShipRouting/scripts/vf_checks.py <vF.xlsx> [--fix] [--skip-shopify]` — report-only default; `--fix` = safe class only (zip/guides/totals/names); exit 1 on FLAG; report to `_outputs/reports/vf_checks_*.md`. Skill: `/vf-checks`. **✅ live-verified 2026-08-28** (`_outputs/reports/vf_checks_AHB_WeeklyProductionQuery_08-31-26_vF_20260828-*.md` ×2) | 🔒 (✍️ only with `--fix`) | 🔴 hand-rolled scratch check scripts (the wk0817 ~5h/~10-run seam: dan_checks/full_sweep/cancel_check — this replaces them); writing gel or routing tags from this path (ice → `ice_distvol_workflow --write`, tags → `vf_tags.py`); rewriting bare `!ANY` rows; "(Tray)" header-substring tray detection (24-vs-321 undercount). Constraints SSOT: `ShipRouting/VF_CHECKS_RULES.md` |
| **RESOLVE a Matrixify add-sheet** (in-sheet + live dupes, removed-line switch, slot-keyed $0 swaps, live-stock floors/caps, swap log) | `AppyHour/scripts/utilities/resolve_matrix_dupes.py --src <export.csv> --out <…_RESOLVED.csv> [--fresh] [--floor SKU=N] [--cap SKU=N] [--zero SKU] [--min-avail 30] [--storm 5]` + `scripts/utilities/order_state_cache.py` (per-order Shopify state, `--fresh` re-pulls mutable box/removed) — READ-ONLY vs Shopify; never overwrites `--out` (versions `-2`,`-3`); writes `<out>_SWAPLOG.txt` every run; exit 2 if the sheet reads as already imported | 🔒 | 🔴 dropping a dupe row (shorts the box — swap the 2nd+ occurrence); a prefix-keyed pool (`AC-` ≠ cracker — the row's `parent_sku` is the slot); re-resolving an already-imported sheet (run the not-landed scan); the older `scripts/resolve_import_dupes.py` (same rules, but no removed-line / slot / live-stock / storm guards); any dated `resolve_dupes_2026_*.py` one-shot. Constraints SSOT: `AppyHour/scripts/RESOLVE_DUPES_RULES.md`; process: `matrixify-import-dupe-check` skill; tests `tests/test_resolve_matrix_dupes.py` |
...
| **READ the MFG-name authority** (SKU → `AHB (S_REG): name`; the ONE resolver for both repos) | `matrix_commander.load_mfg_names()` (table `mfg_names_authoritative`) / `load_mfg_translations()` (table `mfg_translations`) — the DO MySQL tables via `_mfg_from_db`; ShipRouting calls it through `lib.authorities.load_mfg_names`. `--authority <csv>` on any vF tool binds `MFG_AUTHORITY_OVERRIDE` = the ONLY file path | ✅ | 🔴 reading `mfg_names_authoritative.csv` / `mfg_translations.csv` (local READ-MIRRORS, never the authority — Kurt 2026-09-11); a second DB reader; catching `MfgAuthorityUnavailable` into a csv fallback. Write path is the console upload only (`kind=mfg_names`). SSOT: `MATRIX_RULES.md` rule 21 |
| **VALIDATE the MFG-name authority itself** (grammar, dupes, pasted punctuation; delta vs a fresh export) | `scripts/utilities/validate_mfg_authority.py [--diff-against <export.csv>]` — reads the DO table through `load_mfg_names`; `--path` = test file override | ✅ | validating the local csv mirror and calling the table clean |
...
| 🔴 Carrier ACCOUNT identity for `shipments.acct` | `appyhour_lib/acct_canon.py` → `canon(carrier, raw, filename=)`, `sole_account()`, `owner()`, `canonical_keys()` — THE authority (INVOICE_INGEST_RULES §1); one-off accounts (`ONE_OFF`, e.g. FedEx `203180011`) resolve on exact spelling only, never by fuzzy tail/filename match | every writer of `shipments.acct` calls `canon()`; never a raw file/filename/cell value, never a hand-written alias map (`-113`/`-911` split two real accounts across five spellings, 2026-08-20). Asserted by `db_invariants_check` check G |
